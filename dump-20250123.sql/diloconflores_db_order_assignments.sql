...
CREATE TABLE `order_assignments` (
  ...
  `external_carrier` varchar(100) DEFAULT NULL,
  `tracking_number` varchar(100) DEFAULT NULL,
  `status` enum('assigned','in_transit','delivered','failed','cancelled') NOT NULL DEFAULT 'assigned',
  `comments` text,
  ...
  CONSTRAINT `order_assignments_ibfk_2` FOREIGN KEY (`assigned_to`) REFERENCES `users` (`id`) ON DELETE SET NULL
